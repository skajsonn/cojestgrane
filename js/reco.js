// Sekcja „Warto iść” na górze repertuaru.
// Bez klucza Gemini: ranking heurystyczny (watchlista > premiery > oceny TMDB).
// Z kluczem: rekomendacje generuje AI na tym samym kontekście co asystent
// (JSON), z cache'em w localStorage — jedno zapytanie na dzień/aktualizację.

import { el, normalizeTitle, todayIso, dateInfo } from './utils.js';
import { fmtLabel, STATUS_INFO } from './labels.js';
...
function heuristic() {
  const scored = candidates().map((f) => {
    ...
    if (f.tmdb?.voteAverage >= 6.5) { score += f.tmdb.voteAverage * 3; why.push(`dobre oceny (TMDB ${f.tmdb.voteAverage})`); }
    const bigFormats = f.formats.filter((x) => ['imax', '4dx', 'screenx'].includes(x));
    if (bigFormats.length) { score += 6; why.push(`grany w ${bigFormats.map(fmtLabel).join(' i ')}`); }
...
function nextShowing(film) {
  const today = todayIso();
  const cinemaName = Object.fromEntries(data.repertoire.cinemas.map((c) => [c.id, c.name]));
  let best = null;
    ...
    for (const d of Object.keys(byDate)) {
      ...
      for (const s of byDate[d]) {
        ...
        if (!best || key < best.key) {
          best = { key, txt: `najbliższy seans: ${dateInfo(d, today).isToday ? 'dziś' : dateInfo(d, today).dowFull + ' ' + dateInfo(d, today).dayMonth} ${s.time}, ${cinemaName[cid]}${s.formats.length ? ' (' + s.formats.map(fmtLabel).join(', ') + ')' : ''}` };
        }
      }
    }
  }
...
/* ── AI ─────────────────────────────────────────────────────────── */
function cacheKey() {
  return `${data.repertoire.generatedAt}|${data.account ?? '-'}|${getModel()}`;
}
...
async function refreshAi(manual) {
  ...
  try {
    const prompt =
      ...
      `{"title": dokładny polski tytuł z repertuaru, "why": 1–2 zdania po polsku dlaczego dla niego (odwołaj się do jego ocen/gustu), ` +
...
function sanitize(parsed) {
  ...
  return parsed
    ...
    .map((it) => ({
      ...
      why: typeof it.why === 'string' ? it.why.slice(0, 400) : '',
